...
export default function CandidateDashboard() {
  ...
  const stats = [
    { title: t('candidate.total_apps'), value: applications.length, icon: FileText, color: 'bg-indigo-500' },
    { title: t('candidate.pending'), value: applications.filter(a => a.status === 'pending').length, icon: Clock, color: 'bg-amber-500' },
    { title: t('candidate.accepted'), value: applications.filter(a => a.status === 'accepted').length, icon: CheckCircle, color: 'bg-emerald-500' },
    { title: t('candidate.rejected'), value: applications.filter(a => a.status === 'rejected').length, icon: XCircle, color: 'bg-rose-500' },
  ];
  ...
  const recommendedJobs = useMemo(() => {
    ...
    const recommendations = allJobs
      ...
      .map(job => {
        ...
        return { job, matchCount: matchingSkills.length };
      })
    ...
    <div className="max-w-7xl mx-auto px-8 py-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        <div>
          <h1 className="text-3xl font-bold italic text-slate-950 dark:text-white flex items-center gap-3">
             {t('candidate.welcome_back')}, {user?.full_name}! 
             <span className="text-2xl not-italic">👋</span>
          </h1>
          <p className="text-slate-500 font-medium">{t('candidate.whats_happening')}</p>
        </div>
        <div className="flex gap-4">
           <Link to="/profile">
              <Button variant="outline" className="gap-2 bg-slate-50 dark:bg-white/5 border-slate-200 dark:border-white/10 hover:bg-slate-100 dark:hover:bg-white/10"> 
                <Settings className="w-4 h-4" /> {t('candidate.account_settings')}
              </Button>
           </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-12">
           <section className="space-y-6">
             <div className="flex justify-between items-center px-2">
               <div className="flex items-center gap-2">
                 ...
                 <h2 className="text-2xl font-bold text-slate-900 dark:text-white italic">{t('candidate.recommended_for_you')}</h2>
               </div>
               <Link to="/jobs" className="text-indigo-400 text-sm font-bold hover:underline transition-all flex items-center gap-1 group">
                 {t('candidate.explore_all')} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
               </Link>
             </div>
             ...
             <div className="grid grid-cols-1 gap-6">
               ...
               {recommendedJobs.length === 0 && (
                 <div className="p-12 text-center border-2 border-dashed border-white/5 rounded-3xl">
                   ...
                   <p className="text-xs text-slate-500 italic">{t('candidate.browse_jobs_to_learn')}</p>
                 </div>
               )}
             </div>
           </section>
           ...
           <section className="space-y-6">
             <div className="flex justify-between items-center px-2">
               <h2 className="text-2xl font-bold text-slate-900 dark:text-white italic">{t('candidate.recent_applications')}</h2>
               <button 
             ...
             {!isLoading && applications.length === 0 && (
               <div className="text-center py-20 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[2.5rem] backdrop-blur-md">
                 ...
                 <p className="text-lg font-bold text-slate-900 dark:text-white italic">{t('candidate.no_apps_yet')}</p>
                 <p className="text-sm text-slate-500 mb-8">{t('candidate.start_applying')}</p>
                 <Link to="/jobs"><Button className="rounded-full px-8">{t('candidate.browse_all_jobs')}</Button></Link>
               </div>
             )}
           </section>
        </div>
        ...
        <div className="space-y-8">
           <GlassCard className="p-8 text-center bg-slate-50 dark:bg-white/5 border-slate-200 dark:border-white/10" hover={false}>
              ...
              <p className="text-[10px] uppercase tracking-[0.2em] text-slate-500 mb-8 font-black">{t('candidate.verified')} {user?.role}</p>
              
              <div className="space-y-4 mb-8">
                <div className="flex justify-between text-[11px] font-bold text-slate-400 tracking-tight">
                   <span className="italic">{t('candidate.profile_completion')}</span>
                   ...
                   >
                     <div className="flex gap-2 items-start">
                       ...
                       <div className="text-left">
                         <p className="text-[10px] font-bold text-amber-500 uppercase tracking-wider mb-1">{t('candidate.incomplete_profile')}</p>
                         <p className="text-[11px] text-slate-400 leading-tight">{t('candidate.complete_profile_unlock')}</p>
                       </div>
                     </div>
                   </motion.div>
                )}
              ...
              <Link to="/profile">
                <Button className="w-full h-12 rounded-2xl bg-indigo-600 hover:bg-indigo-500 shadow-xl shadow-indigo-600/20">
                  {t('candidate.update_my_profile')}
                </Button>
              </Link>
           </GlassCard>

          <div className="space-y-6">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white italic">{t('candidate.recent_activity')}</h2>
            <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[2rem] overflow-hidden backdrop-blur-md shadow-sm dark:shadow-none">
              <div className="p-8">
                <div className="relative pl-8 border-l border-slate-100 dark:border-white/5 space-y-8">
                  {notifications.length === 0 ? (
                    <div className="text-xs text-slate-500 italic py-4">{t('candidate.no_recent_updates')}</div>
                  ) : (
              ...
              <div className="p-6 bg-slate-50 dark:bg-white/[0.02] border-t border-slate-100 dark:border-white/5">
                <Link to="/settings" className="text-[10px] font-bold text-slate-500 hover:text-slate-950 dark:hover:text-slate-900 dark:text-white transition-colors flex items-center justify-between group">
                  {t('candidate.manage_notifications')}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 lg:p-8 overscroll-contain">
            ...
            >
              <div className="flex items-center justify-between p-6 md:p-8 border-b border-slate-100 dark:border-white/5 shrink-0">
                <h2 className="text-2xl font-bold text-slate-900 dark:text-white italic">{t('candidate.all_your_apps')}</h2>
                <button 
                  onClick={() => setShowAllApplications(false)}
              ...
              <div className="flex-1 overflow-y-auto p-6 md:p-8 bg-slate-50/50 dark:bg-slate-900/50">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {applications.length === 0 ? (
                    <div className="col-span-full py-12 text-center text-slate-500 font-medium">{t('candidate.no_apps_found')}</div>
                  ) : (
