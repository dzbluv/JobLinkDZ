    ...
    <GlassCard className="flex flex-col h-full border-l-4 border-l-indigo-500 bg-white dark:bg-white/5 hover:bg-slate-50 dark:hover:bg-white/[0.08] transition-all group relative">
      <div className="absolute top-4 right-4 flex gap-2 z-10">
         <button 
           ...
           aria-label={favorite ? t('common.remove_favorites') : t('common.add_favorites')}
         >
...
export function ApplicationCard({ application, onDelete }: { application: Application; onDelete?: (id: string) => void }) {
  ...
  const statusConfig = {
    pending: { label: t('common.statuses.pending'), variant: 'warning', icon: Clock, color: 'text-amber-600 dark:text-amber-400' },
    reviewing: { label: t('common.statuses.reviewing'), variant: 'info', icon: Search, color: 'text-indigo-600 dark:text-indigo-400' },
    accepted: { label: t('common.statuses.accepted'), variant: 'success', icon: CheckCircle, color: 'text-emerald-600 dark:text-emerald-400' },
    rejected: { label: t('common.statuses.rejected'), variant: 'error', icon: XCircle, color: 'text-rose-600 dark:text-rose-400' },
  };
    ...
    >
      ...
      <div className="flex items-center gap-4 p-4 mb-6 rounded-2xl bg-slate-50 dark:bg-white/[0.03] border border-slate-100 dark:border-white/5 group-hover:bg-indigo-50 dark:group-hover:bg-indigo-500/5 group-hover:border-indigo-200 dark:group-hover:border-indigo-500/20 transition-all">
        ...
        <div className="flex-1">
          <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] mb-0.5">{t('common.applicant')}</p>
      ...
      <div className="flex items-center justify-between mt-auto pt-5 border-t border-slate-100 dark:border-white/5">
        <div className="flex items-center gap-2 text-slate-500">
          ...
          <p className="text-[10px] font-bold uppercase tracking-widest">
            {t('common.applied')} {new Date(application.created_at).toLocaleDateString()}
          </p>
        </div>
            ...
            <button
              onClick={(e) => {
                ...
                if (application.status === 'rejected' || application.status === 'accepted') {
                  if (window.confirm(t('common.confirm_delete_app'))) {
              ...
              title={
                application.status === 'rejected' || application.status === 'accepted'
                  ? t('common.delete')
                  : t('common.delete_app_restriction')
              }
