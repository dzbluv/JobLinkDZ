...
export function GlassCard({ children, className, hover = true }: { children: React.ReactNode, className?: string, hover?: boolean }) {
  return (
    <motion.div
      ...
      whileHover={hover ? { y: -2, backgroundColor: 'rgba(255, 255, 255, 0.08)', transition: { duration: 0.2 } } : {}}
      className={cn('glass-card p-6 border border-slate-200 dark:border-white/10', className)}
    >
      {children}
    </motion.div>
  );
