  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const fetchedJobs = jobsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as JobOffer));
        const fetchedCompanies = companiesSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Company));
        
        setJobs(fetchedJobs);
        setCompanies(fetchedCompanies);
      } catch (error) {
        handleFirestoreError(error, OperationType.LIST, 'jobs/companies');
      } finally {
        setIsLoading(false);
      }
    }
